...
class ActorCriticAgent:
    def __init__(
        ...
        self.state_size = (
            ...
            order_window_size,
        )  # market_history and order_history
        ...
        self.actor = Actor(
            ...
            output_shape=self.action_space.shape[0],
        )
    ...
    def actor_loss(self, y_true, y_pred):
        ...
        advantages, prediction_picks, actions = (
            y_true[:, :1],
            y_true[:, 1 : 1 + self.action_space.shape[0]],
            y_true[:, 1 + self.action_space.shape[0] :],
        )
        LOSS_CLIPPING = 0.2
        ENTROPY_LOSS = 0.001
        ...
        p2 = (
            torch.clamp(ratio, min=1 - LOSS_CLIPPING, max=1 + LOSS_CLIPPING)
            * advantages
        )
...
class ActorCritic(nn.Module):
    def __init__(
        self, input_shape: tuple, output_shape: int = 256, model: str = "Dense"
    ) -> None:
...
class Actor(nn.Module):
    def __init__(
        self, shared_layers: nn.Module, input_shape: int, output_shape: int
    ) -> None:
